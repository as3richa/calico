mod tuple {
    ...
    mod tests {
        ...
        impl Arbitrary for Tuple {
            fn arbitrary(g: &mut Gen) -> Tuple {
                let Finite(x) = Arbitrary::arbitrary(g);
                let Finite(y) = Arbitrary::arbitrary(g);
                let Finite(z) = Arbitrary::arbitrary(g);
                let Finite(w) = Arbitrary::arbitrary(g);
                Tuple::new(x, y, z, w)
            }
        }
        ...
        fn div(u: Tuple, Finite(a): Finite) -> TestResult {
            if a <= EPSILON {
                ...
                TestResult::from_bool((u / a).eq_approx(Tuple::new(
                    u.x / a,
                    u.y / a,
                    u.z / a,
                    u.w / a,
        ...
        fn norm(u: Tuple) -> bool {
            eq_approx(u.norm(), Float::sqrt(u.dot(u)))
                && eq_approx(u.norm_sq(), u.norm() * u.norm())
        }
        ...
        fn cross(Vector(u): Vector, Vector(v): Vector) -> bool {
            ...
            if eq_approx(u.norm(), 0.0) || eq_approx(v.norm(), 0.0) {
                ...
                eq_approx(s.dot(u) / (u.norm() * sn), 0.0)
                    && eq_approx(s.dot(v) / (v.norm() * sn), 0.0)
            }
        }
        ...
        fn simulate_projectile() -> bool {
            ...
            let total_time = Float::sqrt(100.0 / 4.9);
            let steps = 10000;
